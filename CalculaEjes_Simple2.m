function [d_A_final,d_E2_final,d_B_final] = CalculaEjes_Simple2(Y_simple,Fr,Ft,T,m,Sut,d,Asup,bsup,Conducidas)
    ...
    d_A_prima=sqrt((2*sigma_eq_alt_A)/Se_A);
    ...
    d_A=sqrt((2*sigma_eq_alt_A)/Se_A_nuevo);
    ...
    if m==2 & Conducidas>=12 & Conducidas<=20 & d_E_final<=10
        d_E2_final=10;
    elseif m==2 & Conducidas>=21 & Conducidas<=28 & d_E_final<=12
        d_E2_final=12;
    elseif m==2 & Conducidas>=29 & Conducidas<=40 & d_E_final<=14
        d_E2_final=14;
    elseif m==2 & Conducidas>=41 & Conducidas<=74 & d_E_final<=16
        d_E2_final=16;
    elseif m==2 & Conducidas>=75 & Conducidas<=120 & d_E_final<=20
        d_E2_final=20;
    elseif m==3 & Conducidas>=12 & Conducidas<=15 & d_E_final<=12
        d_E2_final=12;
    elseif m==3 & Conducidas>=16 & Conducidas<=20 & d_E_final<=14
        d_E2_final=14;
    elseif m==3 & Conducidas>=21 & Conducidas<=35 & d_E_final<=16
        d_E2_final=16;
    elseif m==3 & Conducidas>=36 & Conducidas<=69 & d_E_final<=20
        d_E2_final=20;
    elseif m==3 & Conducidas>=70 & Conducidas<=110 & d_E_final<=25
        d_E2_final=25;
    elseif m==3 & Conducidas>=114 & Conducidas<=120 & d_E_final<=30
        d_E2_final=30;
    elseif m==4 & Conducidas>=12 & Conducidas<=15 & d_E_final<=14
        d_E2_final=14;
    elseif m==4 & Conducidas>=16 & Conducidas<=22 & d_E_final<=16
        d_E2_final=16;
    elseif m==4 & Conducidas>=23 & Conducidas<=35 & d_E_final<=20
        d_E2_final=20;
    elseif m==4 & Conducidas>=36 & Conducidas<=75 & d_E_final<=25
        d_E2_final=25;
    elseif m==4 & Conducidas>=76 & Conducidas<=110 & d_E_final<=30
        d_E2_final=30;
    elseif m==5 & Conducidas>=12 & Conducidas<=27 & d_E_final<=20
        d_E2_final=20;
    elseif m==5 & Conducidas>=28 & Conducidas<=49 & d_E_final<=25
        d_E2_final=25;
    elseif m==5 & Conducidas>=50 & Conducidas<=110 & d_E_final<=30
        d_E2_final=30;
    elseif m==6 & Conducidas>=12 & Conducidas<=23 & d_E_final<=20
        d_E2_final=20;
    elseif m==6 & Conducidas>=24 & Conducidas<=40 & d_E_final<=25
        d_E2_final=25;
    else
        f49=msgbox({'El diámetro requerido para el eje de salida es mayor que el diámetro interior del engranaje de salida normalizado que ha sido seleccionado.';' ';...
            ...
            '4. Aumentar la velocidad de salida.'},'Error','error');
        d_E2_final=d_E_final;
    end

    %PUNTO B (final eje)
    %Momento torsor
    tau_Mt_B=(16*T*1000)/pi;
    tau_med_B=KfT*tau_Mt_B;
    %Cortante
    ...
    sigma_eq_med_B=sqrt(3)*tau_med_B;

    %Resistencia a la fatiga
    Se_B=Se_A;

    %Diámetro inicial punto A
    t1_B_prima=(((Se_B*sigma_eq_med_B)/(0.5*Se_B*Sut))+sqrt((((Se_B*sigma_eq_med_B)/(0.5*Se_B*Sut))^2)-4*(((Sut*sigma_eq_alt_B)^3)/(27*((0.5*Se_B*Sut)^3)))))/2;
    t2_B_prima=(((Se_B*sigma_eq_med_B)/(0.5*Se_B*Sut))-sqrt((((Se_B*sigma_eq_med_B)/(0.5*Se_B*Sut))^2)-4*(((Sut*sigma_eq_alt_B)^3)/(27*((0.5*Se_B*Sut)^3)))))/2;
    d_B_prima=nthroot(t1_B_prima,3)+nthroot(t2_B_prima,3);
    ...
    t1_B=(((Se_B_nuevo*sigma_eq_med_B)/(0.5*Se_B_nuevo*Sut))+sqrt((((Se_B_nuevo*sigma_eq_med_B)/(0.5*Se_B_nuevo*Sut))^2)-4*(((Sut*sigma_eq_alt_B)^3)/(27*((0.5*Se_B_nuevo*Sut)^3)))))/2;
    t2_B=(((Se_B_nuevo*sigma_eq_med_B)/(0.5*Se_B_nuevo*Sut))-sqrt((((Se_B_nuevo*sigma_eq_med_B)/(0.5*Se_B_nuevo*Sut))^2)-4*(((Sut*sigma_eq_alt_B)^3)/(27*((0.5*Se_B_nuevo*Sut)^3)))))/2;
    d_B=nthroot(t1_B,3)+nthroot(t2_B,3);
